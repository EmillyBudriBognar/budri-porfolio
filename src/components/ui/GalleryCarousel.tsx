...
const GalleryCarousel: React.FC<GalleryCarouselProps> = ({
    ...
    const handleVideoPlayback = () => {
        if (!autoPlayVideos || !mediaItems[language || 'en']) return;

        const currentMedia = mediaItems[language || 'en'][currentIndex];
    ...
    const { sectionTitle } = translations[language || 'en'] || translations['en'];

    // Acesse os mediaItems corretamente para o idioma selecionado
    const currentMedia = (mediaItems[language || 'en'] || mediaItems['pt'] || [])
        .filter((item): item is MediaItem => !!item && (item.type === 'image' || item.type === 'video'))
        .map(item => ({
            ...item,
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
        ...
        if (touchStartX && Math.abs(e.touches[0].clientX - touchStartX) > 10) {
            // e.preventDefault(); // Cannot preventDefault on React synthetic event easily, normally handled via CSS touch-action
        }
    };
